...
function updateCollectionJsonBasedOnFiles(collectionDir, collectionName) {
    ...
    console.log("Trying to write new file");
...
function toCollectionJsonItemRecursive(collectionDir, items) {
    ...
    dirs.forEach(dir => {
        ...
        if (fs.existsSync(requestFile)) {
            item.item = [];//Needs to be set here in order for newman to register this as runable
            let request = readObjFromJson(requestFile);
            request.url = {
                raw: request.url//MIGHT NEED the other values 
            };
        ...
        toCollectionJsonItemRecursive(collectionDir + "\\" + dir, item.item);
        items.push(item);
    });
}
